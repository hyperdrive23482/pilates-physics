...
import './SpringCalculatorLanding.css'
...
const GET = [
  {
    ...
    body: 'The physics of Pilates springs, instructor to instructor, built right into the tool: why a spring is a range of weights instead of a single one, how a spring’s full range reads as one line on a graph, and the two numbers that define every spring you teach on.',
  },
  {
    n: '02',
    label: 'FREE TOOL',
...
export default function SpringCalculatorLanding() {
  ...
  async function handleClaim() {
    ...
    try {
      ...
      const data = await res.json().catch(() => ({}))
      if (!res.ok) throw new Error(data.error || 'Something went wrong. Try again.')
      navigate('/portal/spring-load-calculator')
    } catch (err) {
      setErrorMsg(err.message || 'Something went wrong. Try again.')
      setStatus('error')
    }
          ...
          {!authLoading && user ? (
            ...
            <div>
              <p className="springs101-signup__lede">
                You are signed in as {user.email}. One click adds the Spring Load Calculator to
                your portal.
              </p>
              <button
                type="button"
            ...
            <div className="springs101-signup__success">
              ...
              <p className="springs101-signup__success-body">
                {result?.emailSent
                  ? `Your sign-in link is on its way to ${email.trim()}. Click it and the Spring Load Calculator will be waiting in your portal.`
            ...
            <>
              <p className="springs101-signup__lede">
                Enter your info and we will set up your free account.
              </p>
