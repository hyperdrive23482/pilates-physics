...
Provider: Pilates Physics
Instructor: Kaleen Canevari
...
Certified Pilates teachers and teachers in training who instruct on the reformer. No prior physics or engineering background is required. All calculations are presented at an arithmetic level with worked examples.
...
- Course premise: modern reformers offer many features for adapting the machine to the individual client, and most teachers use only a fraction of them.
- Course takeaway: the ability to understand how a reformer applies load and what adjustments are available.
- Instructor introduction and background.
- Course outline.

### Module 1: Reformer Anatomy

Objective: establish a common vocabulary for the parts that adjust or deliver load, and the tradeoffs of each.

- Orientation: head end and foot end.
...
- Hooke's Law, F = kx + b: force, spring constant, stretch, and initial tension. The more a spring stretches, the heavier it gets.
- Reformer spring "weights": modern reformers offer three or more, and manufacturers do not share a common standard. Reading a spring resistance chart.
- Interactive spring load calculator for comparing springs across manufacturers.
- Spring life: the rate of wear depends on climate, cleanliness (lotions and oils), amount of use, and abuse.
- Deformation: every stretch creates micro-cracks. Elastic deformation, in which a spring can be cycled many times without losing stiffness, versus plastic deformation, in which a spring does not return to its original form and can lose significant stiffness in a single cycle.
...
Objective: know the levers available to fit the client and change load. Every adjustment comes back to how far the springs stretch.

- Footbar-to-shoulder-rest distance as the governing dimension for fit.
- Footbar: on a pivoting footbar, vertical and horizontal position are coupled, so it cannot be moved closer without also moving higher. Adjusting the footbar changes the range of spring extension and therefore the minimum and maximum resistance. Measured example: footwork on one red spring, footbar mid versus footbar low, showing the change in extension and maximum load.
- Gear bar (spring bar): moves the carriage closer to or farther from the footbar. The closer to the footbar, the greater the possible spring stretch. Measured example: gear 1 versus gear 2 for footwork.
...
Objective: how pulleys change spring load delivery.
...
Objective: understand which mechanical factors influence the "feel" of a reformer.

- Overview of traditional and modern reformer construction.
- Springs: differences in spring type and configuration.
...
Kaleen Canevari is a mechanical engineer and Pilates instructor. She is a former engineer at Balanced Body and provided Pilates equipment maintenance through The Fit Reformer. She founded a smart Pilates equipment company, where she led the design of the Flexia reformer, and sold the company in 2025. She currently teaches Pilates locally, is the founder of Pilates Physics, which teaches the mechanics of Pilates apparatus and loads to Pilates teachers, and is developing Remo, a notetaking app for Pilates teachers. She holds a B.S. in Mechanical Engineering.
...
Six multiple-choice questions. Each question has one correct answer. Correct answers are listed in the key that follows the questions.

**Question 1.** According to Hooke's Law, what happens to a reformer spring's resistance as the carriage moves away from the footbar and the spring stretches farther?

- A. It increases. The farther the spring stretches, the heavier it gets.
- B. It stays constant, because the spring "weight" is fixed by its color.
- C. It decreases, because the spring loses tension as it lengthens.
- D. It increases as it stretches, and as it returns to its resting position.

**Question 2.** Two manufacturers both sell a spring they call "red." Which statement is correct?

- A. Both springs deliver the same resistance because spring colors are standardized across the industry.
- B. The two springs may deliver different resistance, because there is no shared standard and each spring's stiffness is set by its wire diameter, coil diameter, and coil length.
- C. The two springs differ only in coating, which does not affect resistance.
- D. The two springs differ only in initial tension, which does not affect resistance.

**Question 3.** The gear bar (spring bar) is moved so that the carriage sits closer to the footbar. What is the effect on load during footwork?

- A. The springs can stretch farther, so the maximum resistance increases.
- B. The springs stretch less, so the maximum resistance decreases.
- C. Resistance is unchanged because the spring constant has not changed.
- D. Resistance changes only if the footbar is also moved.

**Question 4.** A client holds the straps for supine arm work. Is the force the client feels in the rope the same as the tension in the spring?

- A. No. The pulley system changes the relationship, so the rope force is less than the spring tension at any moment, but it still increases as the spring stretches.
- B. No. The rope force is greater than the spring tension because the pulley multiplies it.
- C. Yes. The rope simply transmits the spring tension to the client's hands unchanged.
- D. Yes, but only when the pulleys are set in the low position.

**Question 5.** A teacher places a 10-pound dumbbell on the carriage to make footwork harder. Does the carriage become 10 pounds harder to move?

- A. Yes. Weight on the carriage adds directly to the resistance.
- B. No. The wheels support the added weight, and it only increases rolling friction by a tiny fraction of a pound. Resistance on the reformer comes from spring stretch, not carriage weight.
- C. Yes, but only on the push out. On the return the dumbbell helps.
- D. No. The added weight makes the carriage easier to move because of momentum.
